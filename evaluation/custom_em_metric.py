"""
Custom EM metric with numeric tolerance.
Mirrors: https://github.com/TableBench/TableBench/blob/main/metrics/custom_em_metric.py
"""
...
def normalize_number(value: str) -> Decimal:
    """Convert string to Decimal, supporting percentages."""
    if value.endswith('%'):
        value = value.strip('%')
...
def get_decimal_precision(values: List[str]) -> int:
    """Return the smallest number of decimal places across a set of reference strings."""
    precisions = []
    for val in values:
        if val.endswith('%'):
            continue
...
def is_number(val: str) -> bool:
    """Return True if val looks like a number or percentage."""
    val = val.strip()
...
def compute_em(references: List[str], predictions: List[str]) -> float:
    """
    Evaluate overall EM considering comma-separated multi-answer strings.
    Mirrors TableBench custom_em_metric.compute_em.
    """
    total_score = 0.0
    total_count = 0
            ...
            if is_number(r):
                try:
                    if r.endswith('%'):
                        ...
                        ref_vals  = [x for x in ref_answers if is_number(x) and not x.endswith('%')]
                        precision = get_decimal_precision(ref_vals)
                        norm_r    = round_decimal(normalize_number(r), precision)
                        norm_p    = round_decimal(normalize_number(p), precision)
                        if norm_r == norm_p:
                            match_score += weight
                except Exception:
                    continue
...
def compute_em_with_tolerance(
    references: List[str],
    predictions: List[str],
    error_range: float,
) -> float:
    """
    EM where numeric answers within `error_range` % of the reference are accepted.
    Mirrors TableBench custom_em_metric.compute_em_with_tolerance.
    """
    total_score = 0.0
    total_count = 0
